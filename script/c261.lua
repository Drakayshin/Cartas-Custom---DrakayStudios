...
function s.initial_effect(c)
    ...
    e0:SetCountLimit(1,{id,0}) -- "Una vez por turno"
    e0:SetTarget(s.sptg)
    ...
    if Duel.DiscardDeck(tp,2,REASON_EFFECT)~=0 then
		...
		if oc:IsLocation(LOCATION_GRAVE) and c:IsRelateToEffect(e) then
            Duel.SpecialSummon(c,0,tp,tp,false,false,POS_FACEUP)
             -- Restricción del Extra Deck (solo Sincronía)
            local e1=Effect.CreateEffect(c)
...
function s.sphfilter(c)
	return c:IsMonster() and c:ListsCode(CARD_BLACK_ROSE_DRAGON) or c:IsCode(CARD_BLACK_ROSE_DRAGON)
end
