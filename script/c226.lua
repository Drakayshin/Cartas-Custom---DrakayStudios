...
function s.thspop(e,tp,eg,ep,ev,re,r,rp)
	...
	if #g>0 and Duel.SendtoHand(g,nil,REASON_EFFECT)>0 then
		...
		if Duel.IsExistingMatchingCard(aux.NecroValleyFilter(s.thspfilter),tp,LOCATION_HAND|LOCATION_GRAVE,0,1,nil,e,tp,ft) and Duel.SelectYesNo(tp,aux.Stringid(id,1)) then
			Duel.Hint(HINT_SELECTMSG,tp,aux.Stringid(id,1))
			local sc=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(s.thspfilter),tp,LOCATION_HAND|LOCATION_GRAVE,0,1,1,nil,e,tp,ft):GetFirst()
