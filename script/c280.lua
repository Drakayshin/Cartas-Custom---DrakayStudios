...
function s.initial_effect(c)
    --  Efecto 0: No puedes Invocar de Modo Especial desde tu Deck Extra
    ...
    --  Efecto 1: Reflejo de daño por batalla a tu adversario
	local e1=Effect.CreateEffect(c)
	e1:SetType(EFFECT_TYPE_SINGLE)
	e1:SetCode(EFFECT_REFLECT_BATTLE_DAMAGE)
	e1:SetValue(1)
    c:RegisterEffect(e1)
    --  Efecto 2: Invocar a esta carta de Modo Especial (desde la mano)
	...
	c:RegisterEffect(e2)
    --  Efecto 3: Invocar a esta carta de Modo Especial desde el Cementerio o destierro
	...
	c:RegisterEffect(e3)
	--  Efecto 4: Añadir a tu mano o Invocar de Modo Especial hasta 2 "Molestor"
...
s.listed_names={100}
	...
	local ATTRIBUTES=ATTRIBUTE_EARTH|ATTRIBUTE_WATER|ATTRIBUTE_FIRE|ATTRIBUTE_WIND|ATTRIBUTE_LIGHT|ATTRIBUTE_DIVINE
function s.spcon(e,c)
	if c==nil then return true end
	local tp=e:GetHandlerPlayer()
	return Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and (Duel.GetFieldGroupCount(tp,LOCATION_MZONE,0,nil)==0  
	or Duel.GetMatchingGroupCount(aux.FaceupFilter(Card.IsAttribute,ATTRIBUTES),tp,LOCATION_MZONE,0,nil)==0)
end
    --  *EFECTO 3°
