...
function s.initial_effect(c)
    ...
    -- 	*Invocar esta carta una vez por turno
	...
	c:RegisterEffect(e0)
	--	1° Debe ser Invocado desde la mano
	...
	c:RegisterEffect(e3)
    -- 	4° Inafectada por efectos de cartas Mágica/ de Trampa
	...
	c:RegisterEffect(e4)
    -- 	5° Ningún jugador puede Invocar de Modo Normales o Especiales
	...
	c:RegisterEffect(e5)
	local e5a=e5:Clone()
	e5a:SetCode(EFFECT_CANNOT_SUMMON)
	c:RegisterEffect(e5a)
    -- 	6° Gana ATK/DEF igual a tus LP
	...
	e6:SetValue(function(e,c) return Duel.GetLP(c:GetControler(1,e)) end)
	c:RegisterEffect(e6)
	local e6a=e6:Clone()
	e6a:SetCode(EFFECT_UPDATE_DEFENSE)
	c:RegisterEffect(e6a)

    -- EFECTO DE PENDULO

	-- 	7° Regresar esta carta a tu mano con otra que controles que mencione "El sello de Oricalcos"
	...
	e7:SetTarget(s.sthtg)
	e7:SetOperation(s.sthop)
	c:RegisterEffect(e7)
	-- 	8° Invocar de Modo Especial 1 monstruo que mencione "El Sello de Oricalcos" y cambialo a objetivo de un ataque
...
function s.penop(e,tp,eg,ep,ev,re,r,rp)
    ...
    -- *Condición general para los 4°, 5° y 6°
...
function s.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingMatchingCard(s.filter,tp,LOCATION_HAND|LOCATION_DECK|LOCATION_GRAVE,0,1,nil,e,tp,lsc,rsc) end
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	local tc=Duel.SelectMatchingCard(tp,s.filter,tp,LOCATION_HAND|LOCATION_DECK|LOCATION_GRAVE,0,1,1,nil,e,tp,lsc,rsc):GetFirst()
